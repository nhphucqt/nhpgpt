...
async function getGPTResult(_promptToRetry, _uniqueIdToRetry) {
    ...
    try {
        ...
        if (model === 'image') {
            ...
            responseData = response.body.getReader();
        }
        if (model === 'image') {
            // Show image for `Create image` model
            responseElement.innerHTML = `<img src="${responseData}" class="ai-image" alt="generated image"/>`
        } else {
            const textDecoder = new TextDecoder("utf-8");
            let responseText = new String();
            while (true) {
                const { value, done } = await responseData.read();
                if (done) {
                    break;
                }
                responseText += textDecoder.decode(value);
                responseElement.innerHTML = await converter.makeHtml(autoFixMarkDown(responseText).trim());
                responseList.scrollTop = responseList.scrollHeight;
